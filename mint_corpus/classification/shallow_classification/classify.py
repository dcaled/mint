...
def load_or_create_preprocessed_corpus(paths_to_subsets, path_to_preprocessed_corpus, mode, max_features=100):
    ...
    # Create preprocessed corpus files.
        ...
        x_train, x_val, x_test = preprocess.vectorize_corpus(x_train, x_val, x_test, max_features=max_features)

        x = {"x_train": x_train, "x_val": x_val, "x_test": x_test}
        y = {"y_train": y_train, "y_val": y_val, "y_test": y_test}

        if not os.path.exists(path_to_preprocessed_corpus):
            os.makedirs(path_to_preprocessed_corpus)
...
def main():
    ...
    path_to_preprocessed_corpus = "../../corpus/preprocessed"
    ...
    # Estimated max features (no spaces, no punctuation).
    MODES_MAX_FEATURES = {
        "headline": 11,
        "body_text": 268,
        "headline_body_text": 273
    }

    for mode in MODES:
        # Loading corpus:
        x, y = load_or_create_preprocessed_corpus(paths_to_subsets, path_to_preprocessed_corpus, mode,
                                                  max_features=MODES_MAX_FEATURES[mode])
        ...
        report.print()
        report.save("{}/{}_{}".format(path_to_results, mode, "random_forest"))
